...
namespace occa {
  ...
  namespace openacc {
    ...
    std::string compilerFlag(const int vendor_,
      ...
      std::stringstream ss;
      std::string flag = openacc::notSupported;
      
      const std::string safeCompiler = removeSlashes(compiler);
      const std::string &hash = safeCompiler;
      
      const std::string testFilename   = sys::getFilename("[occa]/testing/openaccTest.cpp");
      const std::string binaryFilename = sys::getFilename("[occa]/testing/openacc_" + safeCompiler);
      const std::string infoFilename   = sys::getFilename("[occa]/testing/openaccInfo_" + safeCompiler);
      
      cacheFile(testFilename,
                readFile(env::OCCA_DIR + "/scripts/openaccTest.cpp"),
                "openaccTest");
	...
	releaseHash(hash);
      ...
      return "/openacc"; // VS Compilers support OpenMP                          
#endif
    }
  }
  //==================================    
  ...
  kernel_t<OpenACC>* kernel_t<OpenACC>::buildFromSource(const std::string &filename,
    ...
    const std::string openaccFlag = dData_.OpenACCFlag;

    if((dHandle->compilerFlags.find(openaccFlag) == std::string::npos) &&
       (            info.flags.find(openaccFlag) == std::string::npos)){

      info.flags += ' ';
      info.flags += openaccFlag;
    }
    //============================================                                                                            
...
#  endif
  ...
  template <>
  memory_t<OpenACC>::memory_t(const memory_t<OpenACC> &m){
    *this = m;
  }
  
  template <>
  memory_t<OpenACC>& memory_t<OpenACC>::operator = (const memory_t<OpenACC> &m){
    memInfo = m.memInfo;
  ...
  void memory_t<OpenACC>::copyFrom(const memory_v *src,
    ...
    onst void *srcPtr = ((char*) (src->isATexture() ? src->textureInfo.arg : src->handle)) + srcOffset;
...
//==================================   
  ...
  template <>
  device_t<OpenACC>::device_t(const device_t<OpenACC> &d){
    *this = d;
  }

  template <>
  device_t<OpenACC>& device_t<OpenACC>::operator = (const device_t<OpenACC> &d){
    modelID_ = d.modelID_;
    id_      = d.id_;
  ...
  void device_t<OpenACC>::setup(argInfoMap &aim){
    ...
    // Generate an OpenACC library dependency (so it doesn't crash when dlclose())                                  
    acc_get_num_threads();

    data = new OpenACCDeviceData_t;

    OCCA_EXTRACT_DATA(OpenACC, Device);

    data_.vendor         = cpu::compilerVendor(compiler);
    data_.OpenACCFlag     = openacc::compilerFlag(data_.vendor, compiler);
    data_.supportsOpenACC = (data_.OpenACCFlag != openacc::notSupported);
...
#else
#  if OCCA_DEBUG_ENABLED
    compilerFlags = " /Od /openacc";
#  else
    compilerFlags = " /O2 /openacc";
#  endif
...
#  endif
    ...
    if(visualStudioTools != NULL){
      ...
      std::cout << "WARNING: Visual Studio environment variable not found -> compiler environment (vcvarsall.bat) \
maybe not correctly setup." << std::endl;
    }
#endif
  }
  ...
  template <>
  bool device_t<OpenACC>::fakesUva(){
    return false;
  }
  ...
  kernel_v* device_t<OpenACC>::buildKernelFromSource(const std::string &filename,
    ...
    if(data_.supportsOpenACC){
      k = new kernel_t<OpenACC>;
    }
    else{
      std::cout << "Compiler [" << compiler << "] does not support OpenACC, defaulting to [Serial] mode\n";
  ...
  kernel_v* device_t<OpenACC>::loadKernelFromLibrary(const char *cache,
                                                    const std::string &functionName){
#if 0
    kernel_v *k = new kernel_t<OpenACC>;
    k->dHandle = this;
  ...
  memory_v* device_t<OpenACC>::wrapMemory(void *handle_,
                                         const uintptr_t bytes){
    memory_v *mem = new memory_t<OpenACC>;
